...
namespace Getting_Win_Devices
{
    class DeviceInformationSearcher
    {
        ...
        private string[] GetDriverInfo(ManagementObject device)
        {
            ...
            {
                driverInfo[0] += driverParameter["Description"] == null
                    ? ""
                    : (driverParameter["Description"].ToString() + "\n");
                driverInfo[1] += driverParameter["PathName"] == null
                    ? ""
                    : (driverParameter["PathName"].ToString() + "\n");
            }
            return driverInfo;
        }
    }
